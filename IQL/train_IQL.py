...
from IQL import V_IQL, Q_IQL, Policy_IQL_Det
...
def Q_alpha(q_iql, s, a, alpha):
    qs = torch.stack([q(s, a).squeeze(-1) for q in q_iql], dim=0).transpose(1, 0)
...
def V_loss(q_alpha, v_iql, s, tau):
    losses = L_2_tau(q_alpha - v_iql(s), tau=tau)
    return torch.mean(losses)

def Q_loss(q_iql, v_target, s, a, delta):
    q_vals = torch.stack([q(s, a) for q in q_iql], dim=1)
    v_expanded = v_target.unsqueeze(1).expand_as(q_vals)
    losses = mse_loss(v_expanded - q_vals).mean(dim=0).sum()
    
    return losses

def policy_loss(q_alpha, v_next, policy_iql, s, a, beta):
    with torch.no_grad():
        A = q_alpha - v_next
        W = torch.exp(beta * A).clamp(max=100.0)
    losses = W * policy_iql.log_prob(s, a)
    return -torch.mean(losses)

def train_IQL_epoch(q_iql, v_iql, q_iql_target, v_iql_target, policy_iql, dataloader, alpha, tau, gamma, delta, beta, optimizers, device):
    for q in q_iql:
        q.train()
    v_iql.train()
    policy_iql.train()
    ...
    for step, (s, a, r, s_next, done) in enumerate(dataloader):
        ...
        with torch.no_grad():
            q_alpha = Q_alpha(q_iql_target, s, a, alpha)
            v_next = v_iql(s)
            v_target = r + gamma * v_iql_target(s_next) * (1.0 - done.float()).unsqueeze(-1)
        v_loss = V_loss(q_alpha, v_iql, s, tau)
        q_loss = Q_loss(q_iql, v_target, s, a, delta=delta) 
        policy_loss_value= policy_loss(q_alpha, v_next, policy_iql, s, a, beta)
        
        total_loss = v_loss + q_loss + policy_loss_value
        total_loss.backward()
        
        torch.nn.utils.clip_grad_norm_(v_iql.parameters(), max_norm=1.0)
        for q in q_iql:
            torch.nn.utils.clip_grad_norm_(q.parameters(), max_norm=1.0)
        torch.nn.utils.clip_grad_norm_(policy_iql.parameters(), max_norm=1.0)
        v_optimizer.step()
        for q_optimizer in q_optimizers:
            q_optimizer.step()
        ...
        with torch.no_grad():
            for v_target_param, v_param in zip(v_iql_target.parameters(), v_iql.parameters()):
                v_target_param.data.copy_(polyak * v_param.data + (1 - polyak) * v_target_param.data)
            for q_target, q in zip(q_iql_target, q_iql):
...
if __name__ == "__main__":
    ...
    if checkpoint_files:
        ...
        v_iql = V_IQL(state_dim).to(device, non_blocking=True)
        q_iql = [Q_IQL(state_dim, action_dim).to(device, non_blocking=True) for _ in range(K)]
        policy_iql = Policy_IQL_Det(state_dim, action_dim, max_action).to(device, non_blocking=True)

        v_iql.load_state_dict(checkpoint["v_state_dict"])
        for q, q_state_dict in zip(q_iql, checkpoint["q_state_dicts"]):
            q.load_state_dict(q_state_dict)
        policy_iql.load_state_dict(checkpoint["policy_state_dict"])

        v_optimizer = torch.optim.Adam(v_iql.parameters(), lr=lr)
        q_optimizers = [torch.optim.Adam(q.parameters(), lr=lr) for q in q_iql]
        policy_optimizer = torch.optim.Adam(policy_iql.parameters(), lr=lr)
        ...
        q_optimizers = [torch.optim.Adam(q.parameters(), lr=lr) for q in q_iql]
        policy_optimizer = torch.optim.Adam(policy_iql.parameters(), lr=lr)
    v_iql_target = copy.deepcopy(v_iql).to(device, non_blocking=True)
    q_iql_target = [copy.deepcopy(q).to(device, non_blocking=True) for q in q_iql]
    v_iql_target.eval()
    for q in q_iql_target:
        q.eval()    
    ...
    for e in range(epochs):
        mean_v_loss, mean_q_loss, mean_policy_loss, mean_td_target_mean = train_IQL_epoch(
            q_iql, v_iql, q_iql_target, v_iql_target, policy_iql, train_dataloader, 
        ...
        if (e + 1) % 10 == 0:
            returns = evaluate_policy(policy_iql, mean=obs_mean, std=obs_std, env_name=args.env_name, num_episodes=30, max_steps=1000, device=device)
        ...
        if (e + 1) % 10 == 0: # save checkpoint every 10 epochs
            checkpoint = {
            "epoch": e + 1,
            "policy_state_dict": policy_iql.state_dict(),
            "policy_optimizer_state_dict": policy_optimizer.state_dict(),
            "v_state_dict": v_iql.state_dict(),
            "v_optimizer_state_dict": v_optimizer.state_dict(),
            "q_state_dicts": [q.state_dict() for q in q_iql],
        ...
        if (e + 1) % 10 == 0:
            average_return = sum(returns) / len(returns)
            if not hasattr(train_IQL_epoch, "best_return") or average_return > train_IQL_epoch.best_return:
                train_IQL_epoch.best_return = average_return
                best_checkpoint = {
                "epoch": e + 1,
                "policy_state_dict": policy_iql.state_dict(),
                "policy_optimizer_state_dict": policy_optimizer.state_dict(),
                ...
                "q_optimizer_state_dicts": [q_optimizer.state_dict() for q_optimizer in q_optimizers],
                "config": dict(config),
                "best_return": train_IQL_epoch.best_return
                }
